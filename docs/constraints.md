...
| C-META-05 | 作为本仓库 dogfooding 主回路 AI，任何 session 的**第一动作**必须读项目 SSoT：`CLAUDE.md`（写作约定）+ `methodology/12-commit-standards.md`（Co-Authored-By 格式）+ `docs/constraints.md`（C-* 全集与 VH 历史）。任何 session-level harness 默认（如 system prompt 里的 Co-Authored-By 模板、emoji 倾向、退出码验证范式等）与项目 SSoT 冲突时**项目 SSoT 胜**。AI 在第一动作完成前的产出（commit message / 文档 / 验证命令）按"风险输出"对待，进入下一阶段前必须自审一次 | session-level 默认是"对所有项目都对"的通用模板，项目 SSoT 是"对本项目最对"的特化版本；后者优先级永远高。本 session 同 PR 内连续 3 起同类失误（emoji 用了 / pipe 吞退出码 / Co-Authored-By 用 session 默认 trailer）共同根因都是先用默认后忘了核 SSoT（VH-18） | 默认值漂移到本仓库产物，污染 commit history / 文档语料，单次 review 难抓需多模型才能 catch；累积构成"AI 自己不遵守自家约定"的产品反例 |
...
| C-INIT-05 | dogfooding bootstrap 的**首要标志**是仓库根存在 `.claude/settings.json`（per-developer，被 `.gitignore` 排除）。**真正的失效模式不是"settings.json 不热载"而是"settings.json 从未存在"**——SHK 主仓库根默认无项目级 settings，user-level `~/.claude/settings.json` 通常是别的 harness（如 fleet/vibe-island）设置，**与 SHK 自家 28 个 hook 完全互不知情**。因此 bootstrap 流程必须：(a) 主仓库根 `cp templates/settings-json.tmpl .claude/settings.json`（这一步是关键真因，缺它则不论 restart 多少次 hook 都不会跑）；(b) 用户在主仓库根重启 Claude Code session 让 settings 加载；(c) 验证活跃的 3 个标准信号——首次 Bash 触发后 `.harness/observations.jsonl` 与 `tool-count.json` 自动创建 / `echo "chmod 777 X"` 被 safety-guard 拦 exit 2 / 未声明阶段直接 Write 被 stage-guard 拒。`docs/dogfooding-maintainers.md` 必须把 (a) 列为先决步骤，把 restart 列为次要步骤 | VH-18: 上一轮误把"不热载"诊断为根因实测 4 信号一致无效——直到 user 一句"早就重启过无数次了"才纠正方向，发现主仓库根从未存在过 settings.json。这种"症状像但根因错"的诊断陷阱必须被规则化拦截 | 用户以为 dogfooding 在跑（因为 user-level hooks 看似在工作），但 SHK 自家约束完全不生效；commit/edit/stage 各类裸奔；VH-10/VH-16 的"dogfooding 假象"主题再次上演 |
...
| VH-18 | 2026-05-28 | 同一 session 内同一 PR#4 流程出现 4 起结构同构的 dogfooding 失误链：(a) `methodology/18-ci-hardening.md` 写了 4 个 cross-mark emoji（Unicode U+274C，下文同类引用本 VH 时务必用文字代称避免再次违规——VH-17 / VH-18 自己重复掉同一坑），违反 CLAUDE.md "不使用 emoji" 写作约定；(b) PR description 与多个 commit message 声称 validator exit 0，实测 exit 1（VH-17 已细化）；(c) 8 个 commit 的 `Co-Authored-By` trailer 全部用 session-level 默认 `Claude Opus 4.7 (1M context) <noreply@anthropic.com>`，违反 `methodology/12-commit-standards.md` 规定的 `{工具名} ({模型ID})` 格式（工具名必须从 6 类白名单选、模型 ID 只能 `[\w.\-]+`、禁止尾部 email）；(d) 全部 8 个 commit 未经过 6 阶段 Loop 任一阶段声明就裸提交。**共同根因**：作为本仓库 dogfooding 主回路 AI，开 session 第一动作没有读项目 SSoT（CLAUDE.md / methodology/12 / docs/constraints.md），盲信 session-level harness 默认行为（trailer 模板、emoji 倾向、流程跳过）。所有 4 起失误的 review-time 抓取依赖：(a) 自审 Review 阶段抓出（5 项复盘第 5 项）；(b/c) 用户 + 跨模型 review (Claude reviewer + codex reviewer 独立确认)；(d) 用户直接指出"这里需要按 harness 流程来执行"。**附加事件 + 第二轮纠正**：响应"解决 commit-check 不起作用"时第一轮诊断为"Claude Code 不热载 `.claude/settings.json`"——实测 3 信号一致 hooks 未活跃，结论"必须 restart"。**但 user 一句"commit-check 是很久以前就有的机制，早就不知道重启过多少次了"立刻证伪了 restart 假说**——再深查才发现：主仓库根**从未存在过** `.claude/settings.json`（C-INIT-05 强制要求 cp 模板的那一步从未做），user-level `~/.claude/settings.json` 跑的是 vibe-island-bridge（fleet harness）完全不知道 SHK 28 个 hook 存在。**真根因 = 未 bootstrap**，不是 "不热载"。这是经典 VH-10/VH-16 "dogfooding 假象"再次上演：以为在 dogfood、实际 dogfood 主回路从未通电。**单 AI session 自审 4 信号一致仍诊断错方向**，需 user 一句话外部纠正——为 5 层 QA 金字塔第 5 层"人工终审"的必要性提供与 VH-17 codex 反向纠正同等重量的铁证。**修复**：(1) `C-META-05` 强制 dogfooding 主回路 AI 开 session 第一动作必读 SSoT、session 默认与项目 SSoT 冲突时项目胜；(2) `C-INIT-05` 强制 bootstrap 流程含 restart 提示三处（脚本输出 + 维护者文档 + 验证信号）；(3) `methodology/12` 工具/模型表升级到 4.7 当代；(4) `docs/dogfooding-maintainers.md` 新增 restart 步骤与 hook 生效验证信号；(5) 8 个原 commit trailer 已用 filter-branch 重写为合规格式。**强证据**：4 起结构同构事件来自同一 session 同一 AI 同一 PR，证明"AI 单 session 自审多轮也抓不到自己的默认行为漂移"，必须靠 (a) 项目 SSoT 第一动作前置硬约束 + (b) 跨模型 review 兜底，二者缺一不可——为 [04-qa-pyramid.md](../methodology/04-qa-pyramid.md) 第 4 层 Santa 对抗验证的必要性提供铁证 | C-META-05, C-INIT-05, C-TEST-07, C-GATE-05 |
