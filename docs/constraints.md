...
| C-INIT-05 | dogfooding bootstrap 完成的标志是 `.claude/settings.json` 写入仓库根，**但 Claude Code 不在 mid-session 热载该文件**——hooks 注册必须在下一个 session 启动时才生效。因此 bootstrap 流程必须包含**显式 restart 提示**：(a) `bootstrap-dogfood.sh` 之类的脚本最终输出必须有醒目的 `下一步：关闭当前 Claude Code 会话并重新启动，让 hooks 生效` 提示；(b) `docs/dogfooding-maintainers.md` / `SKILL.md` 的 setup 流程必须把 restart 列为 Setup→Verify 的过渡步骤；(c) 验证 bootstrap 是否生效的标准信号是首次 Bash 工具调用时 `.harness/observations.jsonl` 与 `.harness/tool-count.json` 被 hook 创建 | VH-18: bootstrap 文件已写但本 session 实测 hooks 完全未活跃（3 个独立信号一致），用户不知道需要 restart 时会以为"bootstrap 失败" | 用户 init 后以为成功但 hook 实际不生效；后续所有 commit 裸奔；dogfooding 主回路名存实亡 |
