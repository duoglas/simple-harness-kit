...
- **Quality Gate Suite:** `scripts/shk.js` provides `verify` for structured evidence, `doctor` for stage/evidence/hook-enforce health, `security scan` for secrets/leak/config risk, `test-infra assess` for Infra Tier, profile dry-run/repair, and `e2e inspect/bootstrap/plan/run/assess` for app-level E2E bootstrap plus proof strength. `verification-gate.js` reads `.harness/verify-evidence.json` first and requires `overall=READY` before commit/tag gates pass.

- **Phase 2 Quality Engineering Gate:** SHK is an AI harness, not a JS package users operate by hand. Once installed into a target app, it helps Claude Code / Codex do three things: write a short `.harness/iteration-spec.json` spec for the iteration, generate or select meaningful tests, and use `spec status`, `e2e assess`, `test effectiveness`, and `verify` to decide whether delivery is actually ready. E2E PASS is not enough: fake E2E, smoke-only flows, missing assertions, uncovered traffic, or tests that still pass when key logic is broken must be reported as `NOT_SUFFICIENT`. See [`docs/quality-engineering-gate.md`](docs/quality-engineering-gate.md).
- **AI workflow quality gate:** In Claude Code / Codex, SHK skills now tell the agent to classify risk, find tests, decide whether E2E is needed, generate the first E2E for new apps, run the right checks, enter a bounded repair loop on failure, and explain the result in plain language. E2E PASS is not enough by itself: if the E2E is only `echo ok`, only smoke, does not cover the changed risk, or still passes after a critical behavior is broken, SHK reports `NOT_SUFFICIENT` and blocks delivery. The CLI commands are backend checks for the AI, not chores for users to memorize.
...
├── skills/        12 skills (init user-triggered | rest AI-auto)
