...
### 回归类反馈的第一动作是产物 diff，不是假设检验

"昨天还好好的，改完就坏了"——这类反馈的第一步**不是**猜原因（缓存？系统？某个第三方模块？），
而是：

1. 固定两边的真实校验值：当前部署的产物、回滚基线的产物
2. **从目标环境取回实际在跑的那个文件**，与基线做精确 diff
3. 按差异逐条建因果链

在完成版本差异排除之前，不要凭第三方栈把问题归因到缓存、系统或无关模块。
猜测的成本看起来低，但猜错一轮的代价是把真正的差异继续留在那里。

用户的原话很有代表性：「你到底弄了什么才出的问题？你就不能做版本间对比么？」

不适用的情况：纯库、纯文档项目没有"部署产物"概念，这一步自然跳过。
