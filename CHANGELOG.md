...
> 经三轮 Santa 独立对抗审查。第一、二轮判 FAIL（1 critical + 8 high，第二轮的修复未闭合
> critical），第三轮判「HEAD 上没有活的 fail-open，Critical 与 High 真的修好了」，
> 剩余为耐久性问题与一条发布顺序约束。三轮共同的失败模式是**绿灯没有约束力**——
> 测试只测产出侧、修复轮零新单测、断言键拼错被静默忽略。根治手段是给测试框架的
> `expect` 加键名与类型白名单，加上当场就抓出了两个漏登记的真实键。
